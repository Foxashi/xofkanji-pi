...
function fetchStrokeOrder(bodyEl, chars) {
    bodyEl.querySelectorAll('.jisho-stroke-item').forEach((item, i) => {
        const char = chars[i];
        if (!char)
            return;
        fetch(kanjiVgUrl(char))
            .then(r => { if (!r.ok)
            throw new Error(); return r.text(); })
            .then(svgText => {
...
export function initJishoModal() {
...
export function closeJishoModal() {
    const modal = document.getElementById('jisho-modal');
    if (modal)
        modal.style.display = 'none';
}
export async function lookupJisho(query) {
    const modal = document.getElementById('jisho-modal');
    ...
    try {
        ...
        entries.forEach(entry => {
            ...
            if (reading)
                html += '<span class="jisho-reading">【' + escapeHtml(reading) + '】</span>';
            if (jlpt)
                html += '<span class="jisho-tag jisho-jlpt">' + escapeHtml(jlpt) + '</span>';
            if (entry.is_common)
                html += '<span class="jisho-tag jisho-common">common</span>';
            html += '</div>';
