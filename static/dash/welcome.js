...
export function initWelcomeJisho() {
...
export async function loadWelcome() {
    try {
        const [stats, display, daily] = await Promise.all([
            fetchJson('/api/stats'),
            fetchJson('/api/display'),
            fetchJson('/api/daily'),
        ]);
        const totalEl = document.getElementById('welcome-total-kanji');
        if (totalEl)
            totalEl.textContent = String(stats.total_kanji);
