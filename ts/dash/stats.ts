...
import type { StatsData, KanjiItem, LevelKanjiData, DueKanjiItem, DueKanjiData } from '../types.js';

export async function loadStats(): Promise<void> {
    try {
        const data = await fetchJson<StatsData>('/api/stats');
...
export function renderLevels(levels: Record<string, number>): void {
    ...
    if (container.children.length === present.length) {
        present.forEach((level, i) => {
            ...
            const fill = row.querySelector('.level-bar-fill') as HTMLElement | null;
            const count = row.querySelector('.level-count') as HTMLElement | null;
            const pct = max > 0 ? (levels[level] / max) * 100 : 0;
            if (fill) fill.style.width = pct + '%';
            if (count) count.textContent = String(levels[level]);
        });
        return;
    }
...
export async function openLevelModal(level: string): Promise<void> {
    ...
    try {
        const data = await fetchJson<LevelKanjiData>('/api/kanji-by-level/' + encodeURIComponent(level));
        ...
        data.kanji.forEach((k: KanjiItem) => {
        ...
        body.querySelectorAll<HTMLElement>('.modal-kanji-char').forEach(el => {
...
export async function loadDueKanji(): Promise<void> {
    try {
        const data = await fetchJson<DueKanjiData>('/api/due-kanji?limit=100');
        ...
        if (welcomeBadge) welcomeBadge.textContent = String(data.count);
        if (overviewBadge) overviewBadge.textContent = String(data.count);
...
function renderDueKanjiList(due: DueKanjiItem[]): string {
    if (due.length === 0) {
...
function attachDueKanjiClicks(container: HTMLElement): void {
    container.querySelectorAll<HTMLElement>('.due-kanji-char').forEach(el => {
